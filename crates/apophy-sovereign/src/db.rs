...
impl SovereignDb {
    ...
    pub fn fleet_counts(&self) -> Result<(u64, u64, u64)> {
        let (strategic, tactical, operational): (u64, u64, u64) = self.conn.query_row(
            "SELECT
                COALESCE(SUM(CASE WHEN tier = 'strategic' THEN 1 ELSE 0 END), 0),
                COALESCE(SUM(CASE WHEN tier = 'tactical' THEN 1 ELSE 0 END), 0),
                COALESCE(SUM(CASE WHEN tier = 'operational' THEN 1 ELSE 0 END), 0)
             FROM agent_fleet",
            [],
            |row| Ok((
